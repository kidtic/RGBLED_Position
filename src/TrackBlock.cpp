...
TrackBlock::eTrackStatus TrackBlock::track(Mat srcinput)
{
    ...
    if(TrackStatus==PREPARE)
    {
        ...
        rect.x=ledCenter.x-(rect.width/2);
        rect.y=ledCenter.y-(rect.height/2);
    ...
    else if(TrackStatus==SUSPECTED_LOST)
    {
        char rgbcode;
       //S1：对BlockImg进行hsv识别+轮廓点检测
        vector<Point2f> r_center,g_center,b_center;
        ...
        else if(codeStatus==READING)
        {
            ...
            {
                if(rgbcode=='G' || rgbcode=='B') TrackStatus=OK;
                mpReadCodeImg++;
            }
        }
...
void TrackBlock::findContourCenterRGB(Mat inputImg ,vector<Point2f> &r,vector<Point2f> &g,vector<Point2f> &b,vector<double> &Sarea)
{
    ...
    //r
    vector<Point2f> rr;
    findContours(mask_r, contours,hierarchy,CV_RETR_EXTERNAL,CV_CHAIN_APPROX_NONE);
    ...
    //g
    vector<Point2f> gg;
    findContours(mask_g, contours,hierarchy,CV_RETR_EXTERNAL,CV_CHAIN_APPROX_NONE);
    for (int i = 0; i < contours.size(); i++) 
    {	
        //计算轮廓的矩
        Moments M = moments(contours[i]);
    ...
    //b
    vector<Point2f> bb;
    findContours(mask_b, contours,hierarchy,CV_RETR_EXTERNAL,CV_CHAIN_APPROX_NONE);
    for (int i = 0; i < contours.size(); i++) 
    {	
